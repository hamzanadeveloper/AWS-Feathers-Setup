...
module.exports = function (_config) {
  ...
  function register(server, options, next) {
    ...
    var transport = function transport(provider, req, res, session) {
      return function (data) {
        if (!provider.callback) {
          ...
          session.response = data;(req.session || req.yar).set('grant', session);
    ...
    server.route({
      ...
      handler: function handler(req, res) {
        ...
        if (provider.oauth === 1) {
          oauth1.access(provider, session.request, query).then(response).catch(response);
        } else if (provider.oauth === 2) {
          oauth2.access(provider, query, session).then(response).catch(response);
        } else {
